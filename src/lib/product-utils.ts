...
import { isProductOutOfStock, resolveDisplayStock, resolveVariantStockValue } from "./storefront-stock";
...
export function getAvailableStock(product: Product, variantId?: string | null): number {
  const variant = getVariant(product, variantId);
  if (variant) {
    return resolveVariantStockValue(variant);
  }

  return resolveDisplayStock(product);
}
